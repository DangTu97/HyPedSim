...
global {
	...
	float step <- 0.1;
	int MAX_DELAY <- 5;
	float PROB_CONSTANTINE <- 0.4;
	float PROB_CHENAVARD <- 0.5;
	
	// CC model
	float MAXIMUM_SPEED <- 1.4 #m/#s;
	float MINIMUM_SPEED <- 0.2 #m/#s;
	...
	float A <- 3.0;
	float B <- 0.3;
	float preferred_speed <- 1.34 #m/#s;
	float reaction_time <- 0.54 #s;
...
//parallel: true
//
experiment crowd_exit_calibration type: batch repeat: 1 keep_seed: true until: length(individual) = 0  or cycle > 3801  {
